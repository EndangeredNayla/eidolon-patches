...
public class CruciblePage extends Page {
    ...
    public void render(CodexGui gui, PoseStack poseStack, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < steps.length; i ++) {
            ...
            gui.blit(poseStack, tx, ty, 128, 0, 128, 20);
            tx += 24;
            for (int j = 0; j < steps[i].stacks.length; j ++) {
                if (!steps[i].stacks[j].isEmpty()) {
                    gui.blit(poseStack, tx, ty + 1, 176, 32, 16, 17);
                    tx += 17;
                }
            }
            for (int j = 0; j < steps[i].stirs; j ++) {
                gui.blit(poseStack, tx, ty + 1, 192, 32, 16, 17);
                tx += 17;
            }
        }
        gui.blit(poseStack, x, y + yoff + steps.length * 20, 128, 64, 128, 32);

        Font font = Minecraft.getInstance().font;
        for (int i = 0; i < steps.length; i ++) {
            int tx = x, ty = y + yoff + i * 20;
            drawText(gui, poseStack, I18n.get("enchantment.level." + (i + 1)) + ".", tx + 7, ty + 17 - font.lineHeight);
            tx += 24;
        }
    }
